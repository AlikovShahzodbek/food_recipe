...
class _CustomWrapState extends State<CustomWrap> {
  ...
  void initState() {
    super.initState();
    SharedPreferences.getInstance().then((saveIndex) {
      setState(() {
        selectedIndex =
            saveIndex.getInt(widget.indexKey) ?? widget.selectedIndex;
      });
    });
  }
...
void resetIndices() async {
  final prefs = await SharedPreferences.getInstance();
  await prefs.remove('time_index');
  await prefs.remove('rate_index');
  await prefs.remove('category_index');
}
